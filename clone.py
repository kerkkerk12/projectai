...
def main() :
    try:
        ...
        for i in range(len(list_encode)) :
            face_distances = face_recognition.face_distance([list_encode[i][0]], list_encode[i][1])
            result = face_recognition.compare_faces([list_encode[i][0]], list_encode[i][1])
            list_distance.append(face_distances)
            list_compare.append(result)

        try:
            percentage = 1 - np.average(list_distance)
            match = np.median(list_compare)
            if (match):
                match = True
            else:
                match = False
            ...
            cv2.putText(
                frame, ("Percent:" + str('{:.2f}'.format(percentage *100)) + f" Match: {match}"),(10,50),cv2.FONT_HERSHEY_SIMPLEX,1,(209, 80, 0, 255),3)
            ret, buffer = cv2.imencode('.jpg', frame)
            f = buffer.tobytes()
            yield (b'--frame\r\n'
                    b'Content-Type: image/jpeg\r\n\r\n' + f + b'\r\n')
